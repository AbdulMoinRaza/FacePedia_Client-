...
import { Box, useMediaQuery,  InputBase , useTheme, Fab} from "@mui/material";
...
const handleBackSpace = ()=>{
  ...
  const chatTopDiv = document.querySelector(".chatBoxTop");
  const handleScroll = () => {
    if (!chatTopDiv) return;
    const atBottom = chatTopDiv.scrollHeight - chatTopDiv.scrollTop - chatTopDiv.clientHeight < 100;
    setShowScrollToBottom(!atBottom);
  };

  chatTopDiv?.addEventListener("scroll", handleScroll);
  return () => chatTopDiv?.removeEventListener("scroll", handleScroll);
...
<div
  ...
  style={{
    ...
    width: '100%',
    backgroundImage: `url("/assets/${mode !== 'dark' ? 'chat_bg_2.png' : 'chat_bg.png'}")`,
    backgroundSize: 'cover',
    backgroundRepeat: 'no-repeat',
    backgroundPosition: 'center',
...
>
  ...
  <Fab
    onClick={() =>
      document.querySelector(".chatBoxTop").scrollTo({ top: document.querySelector(".chatBoxTop").scrollHeight, behavior: 'smooth' })
    }
    sx={{
      position: "absolute",
      bottom: "5rem",
  ...
  >
    <KeyboardDoubleArrowDownIcon style={{ color: "#fff" }} />
      ...
      {isNonMobileScreens && (
        <Box flexBasis="23%" m="0 0 0 0" p = '0 0 0 0' >
